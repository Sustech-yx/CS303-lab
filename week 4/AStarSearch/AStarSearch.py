...
def AStarSearch(Graph, start, end, distances):
    ...
    pq = q.PriorityQueue()
    visited = []
    g_ = {u: float("inf") for u in all_nodes}
    queue.append(all_nodes[0])
    g_[all_nodes[0]] = 0
    pq.put((g_[all_nodes[0]] + distances[all_nodes[0]], all_nodes[0]))
    ...
    while pq.empty() is not True:
        ...
        for pair in Graph[vertex]:
            ...
            if now not in visited:
                pre = g_[now]
                post = g_[vertex] + pair[1]
                g_[now] = min(post, pre)
                pq.put((g_[now] + distances[now], now))
                queue.append(now)
