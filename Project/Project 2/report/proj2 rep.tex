...
Formally, the objective fundtion of CARP is:
\begin{equation}
\begin{aligned}
minimize TC(s)\\
\sum^m_{k=1}l_k=&|T|\\
\tau_{k_1i_1}&\not= \tau_{k_2i_2}, \forall (k_1, i_1 \not=k_2,i_2)\\
\tau_{k_1i_1} &\not=inv(\tau_{k_2i_2}), \forall(k_1, i_1 \not=k_2,i_2)\\
\sum^{l_k}_{i=1}d(\tau_{ki})&\le Q,\forall k=1,2,...,m\\
\tau_{ki} &\in T
\end{aligned}
\end{equation}

The first line means minimize the total cost of all routes. The first to the third line imply that each task is served exactly once. The fifth line imply that there is capacity constraint. The sixth line imply the definition of $\tau$. $head(\tau)$ and $tail(\tau)$ represent the endpoints of task $\tau$ and specifies the direction from which $\tau$ is served, and $inv(\tau)$ represent the inverse direction. $^{[1]}$

\subsection{Detail of Algorithms}
For more detail of the algorithms, I may start from the algorithm for getting the shortest distance between any two nodes. 

\begin{breakablealgorithm}
\begin{algorithmic}[1]
\Require Instance
\Ensure The distance matrix for the instance
\Function{calDistance}{instance}
	\State graph $\gets$ build\_graph(instance.edge)
	\State n $\gets$ $|$ instance.edge $|$
	\For {k=1 to n}
		\For {i=1 to n}
			\For {j=1 to n}
				\State graph[i][j] = min(graph[i][j], graph[i][k] + graph[k][j])
			\EndFor
		\EndFor
	\EndFor
\EndFunction
\end{algorithmic}
\caption{The algorithm to get the distance matrix. (Floyd)}
\label{alg0}
\end{breakablealgorithm}


With the distance matrix, then I can use it to obatin an initial solution.
